...
for k in thetas:
   collective_pitch=k
   def gen_fun(omega,gamma):
        #theta = collective_pitch 
        ...
        # lift coefficient
        #c_l1 = - 2.56460052e+01*alpha1**3 - 4.43234451e-02*alpha1**2 + 7.00607112*alpha1 + 1.02584291e-03
        #c_l2 = - 2.56460052e+01*alpha2**3 - 4.43234451e-02*alpha2**2 + 7.00607112*alpha2 + 1.02584291e-03
        #c_l3 = - 2.56460052e+01*alpha3**3 - 4.43234451e-02*alpha3**2 + 7.00607112*alpha3 + 1.02584291e-03
        ...
        # drag coefficient
        #c_d1 = 9.40977430e+01*alpha1**6 -1.82128683e-01*alpha1**5 - 8.20854743*alpha1**4 + 1.84470128e-02*alpha1**3 + 4.39400675e-01*alpha1**2-3.71962200e-04*alpha1 + 5.74006568e-03
        #c_d2 = 9.40977430e+01*alpha2**6 -1.82128683e-01*alpha2**5 - 8.20854743*alpha2**4 + 1.84470128e-02*alpha2**3 + 4.39400675e-01*alpha2**2-3.71962200e-04*alpha2 + 5.74006568e-03
        #c_d3 = 9.40977430e+01*alpha3**6 -1.82128683e-01*alpha3**5 - 8.20854743*alpha3**4 + 1.84470128e-02*alpha3**3 + 4.39400675e-01*alpha3**2-3.71962200e-04*alpha3 + 5.74006568e-03
...
#        return F(y,t*y ,[r_0,R])/(m*R**2)
   ...
   print('gamma_angle',gamma_angle)


#print('gamma',gamma)

#print(sol.y)
#print('print t',sol.y[0,1])
#print('gamma',gamma)

   print('END_')

#plt.plot(sol.t,np.unwrap(gamma_angle))

   gamma_angle = ( gamma_angle + np.pi) % (2 * np.pi ) - np.pi
#plt.plot(sol.t,gamma_angle)
   print(sol.t[0])
   for i in range(0,len(sol.y)):
       plt.plot(sol.t,sol.y[i,:]/(np.pi)*180*0.2)
   #plt.plot(sol.t,sol.y[0,:]/(np.pi)*180)

   print('size',gamma_angle.size)
